...
def trainSRU(model, trainingData, device, numBatches, batchSize, blk_size, predictedIdx, max_iter,
    ...
    stop_time = min(start_time + blk_size - 1, numTotalSamples - 1, trainingData.shape[1] - 1)
    ...
    for epoch in range(max_iter):
        ...
        for tt in range(start_time, stop_time, 1):
            ...
            smooth_loss = (1 / (blk_size - 1)) * mseLoss(torch.flatten(model.y_t),
                                                         torch.unsqueeze(trainingData[predictedIdx, tt + 1], 0))
        ...
        if (proxUpdate):
            ...
            with torch.no_grad():
                ...
                model.lin_xr2phi.weight.data[:, :n] = inpWgtMtx * (
                        softshrink2(l2normTensor) / torch.clamp(l2normTensor, min=1e-8))
                ...
                loss1 = lambda1 * ((torch.norm(model.lin_y.weight.data, 1) + torch.norm(model.lin_y.bias.data, 1) +
                                    torch.norm(model.lin_xr2phi.weight[:, n:].data, 1)) + torch.norm(
                    model.lin_xr2phi.bias.data, 1) +
        ...
        # if (printEpoch == 1):
        #     print(
        #         'Predicted Node = %d \t epoch = %s \t lr = %.4f \t Training loss = %.4f \t Fit error = %.4f \t Delta = %f' % (
